...
import 'manage_staff_screen.dart'; // Import skrin manage staff
...
class HomeDashboardScreenState extends State<HomeDashboardScreen> {
  ...
  List<Map<String, dynamic>> topSalesList = [];
  ...
  void initState() {
    ...
    fetchUserRole(); // <-- Panggil fetch role
    fetchTopSalesTodayList();
  }
  ...
  Future<void> fetchTopSalesTodayList() async {
    final result = await _bookService.getTopSalesTodayList();
    setState(() {
      topSalesList = result;
    });
  }
            ...
            : SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    Stack(
                      children: [
                        BookDashboard(
                          ...
                          userRole: userRole,
                          totalDailySales:
                              totalDailySales, // Hantar data sebenar
                          topSalesList: topSalesList,
                        ),
                        Positioned(
                          top: 8,
                          left: 0, // Ubah dari right: 0 ke left: 0
                          child: IconButton(
                    ...
                    // Buang butang View Agents/ElevatedButton.icon warna biru dari sini
                    // (Tiada lagi butang admin di sini)
                  ],
                ),
              ),
      ),
